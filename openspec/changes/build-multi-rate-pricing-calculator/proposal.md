...
- Add email/password authentication, protected application routes, server-derived identity, and PostgreSQL row-level security so users can access only their own data.
...
- Add draft editing with an explicit Save action, spreadsheet-like keyboard behavior, finalization, API-enforced immutability, deletion, and duplication of either draft or finalized documents into new drafts.
...
- Requires a Supabase project with email/password authentication plus a deployment target such as Vercel.
