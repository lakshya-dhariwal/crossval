...
Draft owners SHALL be able to add, edit, and remove line items. Positions SHALL remain deterministic and contiguous after insertion or deletion, and all affected totals SHALL be returned after a successful mutation.
...
- **WHEN** an owner deletes a line from a draft
- **THEN** remaining lines are resequenced, totals are recalculated, and the response identifies a sensible neighboring line for focus restoration
