...
The system MUST protect all document, report, PDF, export, and REST API surfaces with an authenticated Supabase session. Server-rendered routes and route handlers SHALL determine identity from the verified session, and session cookies SHALL be refreshed through the supported server-side Supabase flow.
