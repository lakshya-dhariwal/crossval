  ...
  try {
    return await withUser(request, async (userId, db) => {
      ...
      if (typeof body.version !== "number")
        throw new AppError("VALIDATION_ERROR", "Version is required.", 422);
      const { version, ...raw } = body;
      return ok(
        await updateLine(userId, id, lineItemId, raw as never, version, db),
      );
    });
  } catch (error) {
...
export async function DELETE(request: Request, { params }: Params) {
  ...
  try {
    return await withUser(request, async (userId, db) => {
      ...
      if (typeof body.version !== "number")
        throw new AppError("VALIDATION_ERROR", "Version is required.", 422);
      await deleteLine(userId, id, lineItemId, body.version, db);
