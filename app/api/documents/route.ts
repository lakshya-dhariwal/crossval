...
export async function GET(request: Request) {
  return withUser(request, async (userId, db) => {
    ...
    if (!["all", "draft", "finalized"].includes(status))
      throw new AppError("VALIDATION_ERROR", "Invalid status filter.", 422);
    return ok(
      await listDocuments(
        userId,
        url.searchParams.get("search") ?? "",
        status as "all" | "draft" | "finalized",
        db,
      ),
    );
  });
...
export async function POST(request: Request) {
  try {
    return await withUser(request, async (userId, db) => {
      ...
      return ok(await createDocument(userId, parsed.data, db), 201);
    });
  } catch (error) {
