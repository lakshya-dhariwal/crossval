...
export function assertDecimalToken(
  value: string,
  field: string,
  scale: number,
) {
  if (!value || !DECIMAL.test(value) || value.split(".")[1]?.length > scale)
    throw new CalculationError(field, "Enter a valid decimal value.");
}

export function calculateLineItem(raw: RawLineItem): CalculatedLineItem {
  try {
    const quantity = decimal(raw.quantity);
    const unitPrice = decimal(raw.unitPrice);
    if (quantity.lt(1))
      throw new CalculationError("quantity", "Quantity must be at least 1.");
    if (unitPrice.lt(0))
      throw new CalculationError("unitPrice", "Unit price cannot be negative.");
    ...
    const subtotal = new Decimal(quantity)
      .mul(unitPrice)
    ...
    if (raw.discountType === "percentage" && discountValue.gt(100))
      throw new CalculationError(
        "discountValue",
        "Discount must be between 0 and 100.",
      );
    if (raw.discountType === "none" && !discountValue.isZero())
      throw new CalculationError(
        "discountValue",
        "A line without a discount must use 0.",
      );
    ...
    const discountedAmount = subtotal
      .minus(discountAmount)
      .toDecimalPlaces(2, Decimal.ROUND_HALF_UP);
    const taxAmount = discountedAmount
      .mul(taxPercent)
      .div(100)
      .toDecimalPlaces(2, Decimal.ROUND_HALF_UP);
    ...
    return {
      ...raw,
      discountValue: raw.discountType === "none" ? "0" : raw.discountValue,
      subtotal: subtotal.toFixed(2),
      discountAmount: money(discountAmount),
      discountedAmount: money(discountedAmount),
    ...
    throw new CalculationError(
      "line",
      "Line item contains an invalid numeric value.",
    );
  }
}
...
export function calculateDocument(lines: CalculatedLineItem[]): DocumentTotals {
  const sum = (
    key: keyof Pick<
      CalculatedLineItem,
      "subtotal" | "discountAmount" | "taxAmount" | "lineTotal"
    >,
  ) =>
    lines
      .reduce((total, line) => total.plus(line[key]), new Decimal(0))
      .toDecimalPlaces(2, Decimal.ROUND_HALF_UP)
      .toFixed(2);
  return {
