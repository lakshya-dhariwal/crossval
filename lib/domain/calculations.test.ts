...
describe("pricing calculations", () => {
  it("calculates the assignment sample", () => {
    ...
    expect(lines.map((line) => line.lineTotal)).toEqual([
      "189.00",
      "52.50",
      "180.00",
    ]);
  ...
  it("allows a fixed discount equal to subtotal and preserves fractional inputs", () => {
    ...
    expect(zero.discountedAmount).toBe("0.00");
    expect(zero.taxAmount).toBe("0.00");
    expect(zero.lineTotal).toBe("0.00");
    expect(
      calculateLineItem({
        description: "",
        quantity: "1.25",
